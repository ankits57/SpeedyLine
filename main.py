...
def main_screen():
    ...
    canvas2 = Canvas(main_screen, width=1000, height=500, relief='raised', bg="white")
    canvas2.pack()
    ...
    def registration():
        ...
        relpho2 = StringVar()
        bg = StringVar()
        registration = Toplevel(main_screen)
    ...
    registration = Button(main_screen, text='Registration', command=registration, bg='black', fg='white',font=('helvetica', 25, 'bold'))
    ...
    def fetch():
        ...
        uploadaadhaar = Button(fetch, text='Get Detail By Aadhaar', command=submitaadhaar,
    ...
    fetch = Button(main_screen, text='      Fetch      ', command=fetch, bg='black', fg='white',font=('helvetica', 25, 'bold'))
    canvas2.create_window(850, 150, window=fetch)

    def exit():
        main_screen.destroy()

    exit = Button(main_screen, text='      Exit      ', command=exit, bg='red', fg='white', font=('helvetica', 25, 'bold'))
...
aadhaar_no = StringVar(value="2000XXXX4589")
password = StringVar(value="************")
otp = StringVar(value= "2104")

canvas1 = Canvas(root, width=700, height=100, relief='raised', bg="white")
canvas1.pack()

label1 = Label(root, text='SPEEDY LINE')
label1.config(font=("bold", 60,), bg="white", fg ="#B49FDC")
canvas1.create_window(350, 50, window=label1)

canvas2 = Canvas(root, width=1000, height=500, relief='raised', bg="white")
canvas2.pack()

label2 = Label(root, text='Enter Your Aadhaar No. , Password and Place your finger')
label2.config(font=("bold", 15), bg="white", fg="#F197C0")
canvas2.create_window(450, 20, window=label2)

label3 = Label(root, text='Aadhaar No.')
label3.config(font=("bold", 20), bg="white")
canvas2.create_window(190, 70, window=label3)

entry1 = Entry(root, textvar=aadhaar_no, font=(25), borderwidth=3, width=25)
canvas2.create_window(450, 70, window=entry1)

label4 = Label(root, text='Password')
label4.config(font=("bold", 20), bg="white")
canvas2.create_window(170, 130, window=label4)

entry2 = Entry(root, textvar=password,show ="*", font=(25), borderwidth=3, width=25)
canvas2.create_window(450, 130, window=entry2)
...
def upload():
    ...
    if (filename):
        ...
        img.save('C:\\Users\\ankit\\PycharmProjects\\SpeedyLine\\Upload\\101.bmp', 'BMP')
        img = ImageTk.PhotoImage(img)
        label = Label(root, image=img, width=100, height=100, bg="white")
        canvas2.create_window(750, 100, window=label)


uploadfinger = Button(root, text='Upload Finger', command=upload, font=('helvetica', 14, 'bold'))
canvas2.create_window(750, 200, window=uploadfinger)
    ...
    for i in range(st, 100):
        try:
            ...
            if (score>=20.0):
                ...
                msg = Label(root, text='Fingerprint Record Detected')
                msg.config(font=("bold", 10), bg="white")
                canvas2.create_window(750, 270, window=msg)
...
fetchfinger = Button(root, text='Match Fingerprint', command=submitfinger, font=('helvetica', 14, 'bold'))
canvas2.create_window(750, 310, window=fetchfinger)
def check():
    try:
        mydb = mysql.connector.connect(
        ...
        # a basic sql query to retrieve image from database
        # images are stored as BLOB file in database
        mycursor.execute("SELECT aadhar FROM people WHERE aadhar='%s'" % int(aadh))
        aadhar_data = mycursor.fetchone()
        aadhar_data = tuple(map(str, aadhar_data))
        def convertTuple(tup):
            str = ''
            for item in tup:
                str = str + item
        ...
        aadhar_data = convertTuple(aadhar_data)
        aadhar_data = str(aadhar_data)

        mycursor.execute("SELECT password FROM people WHERE aadhar='%s'" % int(aadh))
        passwd_data = mycursor.fetchone()
        ...
        passwd_data = convertTuple(passwd_data)
        passwd_data = str(passwd_data)

        if(entry1.get() == aadhar_data and entry2.get() == passwd_data):
            print("correct")
            main_screen()
...
check = Button(root, text='Log In', command=check, bg='#A5F8CE', fg='black',  font=('helvetica', 15, 'bold'))
canvas2.create_window(400, 240, window=check)
